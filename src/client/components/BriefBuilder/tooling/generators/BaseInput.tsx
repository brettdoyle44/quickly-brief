...
export default function (props: {
  ...
  const textClass = item.help
    ? "focus-within:not-sr-only"
    : "focus-within:not-sr-only sr-only";

  const [helpTextClass, setHelpTextClass] = useState(textClass);
  const onHelpTextBlur = (e: { target: { textContent: any } }) => {
    ...
    <div className="relative">
      <div className="mb-3">
        <div className="flex">
          <label
            onBlur={(e) => {
              props.onInputChange(e.target.textContent, "label");
            }}
            contentEditable
          ...
          {item.required && <span className="text-gray-500">*</span>}
        </div>
        <div className={helpTextClass} ref={helpTextEl}>
          <div>
            ...
            >
              <p data-placeholder="Write a help text" className="is-empty">
                {item.help}
              </p>
            </div>
          </div>
