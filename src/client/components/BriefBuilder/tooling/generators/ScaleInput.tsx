...
export default function (props: { onInputChange?: any; item?: any }) {
  ...
  const onButtonBlur = (e: { relatedTarget: any }) => {
  ...
  const onButtonOver = (idx: number) => {
    const clickedIdx = buttonList.map((ele) => ele.clicked).indexOf(true);
    if (clickedIdx !== -1) {
      buttonList.forEach((ele) => {
        if (
          (idx <= ele.value && ele.value <= clickedIdx) ||
          (ele.value >= clickedIdx && ele.value <= idx)
        ) {
          ele.classList = "bg-blue-100";
          ele.checked = true;
        } else {
  ...
  return (
    <div onBlur={onButtonBlur} ref={outerRef}>
      <BaseInput item={scaleItem} onInputChange={props.onInputChange}>
          ...
          {buttonList.map((btn) => {
            return (
              <button
                ...
                onMouseEnter={() => {
                  onButtonOver(btn.value);
                }}
              >
                <div className="flex justify-center items-center theme-border theme-ring rounded cursor-pointer border">
          ...
          <div className="row-start-2 col-span-11 grid grid-cols-3">
            <div className="flex justify-start">
              <p
                ...
                onBlur={(e) => {
                  props.onInputChange(e.target.textContent, "from_label");
                }}
            ...
            <div className="flex justify-center">
              <p
                ...
                suppressContentEditableWarning
                onBlur={(e) => {
                  props.onInputChange(e.target.textContent, "center_label");
                }}
                data-placeholder="Add label"
              >
            ...
            <div className="flex justify-end">
              <p
                ...
                data-placeholder="Add label"
                onBlur={(e) => {
                  props.onInputChange(e.target.textContent, "to_label");
                }}
              >
                {scaleItem.to_label}
              </p>
